...
show_mentors = """SELECT mentors.first_name, mentors.last_name,
                                schools.name, schools.country FROM mentors
                                JOIN schools ON mentors.city = schools.city
                                ORDER BY mentors.id ASC"""

show_all_schools = """SELECT mentors.first_name , mentors.last_name,
                                schools.name, schools.country FROM mentors
                                FULL JOIN schools ON mentors.city = schools.city
                                ORDER BY mentors.id ASC"""

show_mentors_by_country = """SELECT schools.country, COUNT(mentors.id) AS count FROM mentors
                            JOIN schools ON mentors.city = schools.city
                            GROUP BY schools.country
                            ORDER BY schools.country ASC"""

show_contacts = """SELECT DISTINCT schools.name, mentors.first_name, mentors.last_name
                    FROM schools LEFT JOIN mentors ON schools.contact_person = mentors.id
                    ORDER BY schools.name"""

show_applicants = """SELECT applicants.first_name, applicants.application_code,
